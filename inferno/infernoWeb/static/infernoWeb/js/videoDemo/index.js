$( document ).ready(function() {
	function pagination(category, length){
		...
		for(var i=start-1;i<end;i++){
			...
			item.attr('href', `/infernoWeb/videoDemo?start=${i*10+1}&category=${category}`)
			$('.borderless.menu').append(item)					
		}
		$('.borderless.menu').append($(`<a class="item" id='turnright'><i class="angle right icon"></i></a>`))

		$('#turnleft').click(function(){
			window.location.href = `/infernoWeb/videoDemo?start=${parseInt(startID)-10}&category=${category}`
		})
	...
	$.getJSON('/videoDemo/get/jcategory', function(json){
		for(let category of json){
			let tmp = $(`<a class="item" href="/infernoWeb/videoDemo?start=1&category=${category['fields']['name']}">${category['fields']['name']}</a>`)
			$('#kind').append(tmp)
		}
	})

	$.getJSON('/videoDemo/get/jlist' + window.location.search, function(json){
		first = json.shift()
		...
		for(let i of json){
			...
			result
			  .find('a.avatar').attr('href', '/infernoWeb/videoDemo/inside?id='+i['id']).end()
