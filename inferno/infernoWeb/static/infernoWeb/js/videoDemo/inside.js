...
$( document ).ready(function() {
    ...
    async function updateRadar(){
      function getjobValue(){
          ...
          return new Promise(function(resolve, reject){
              $.getJSON( "/videoDemo/get/jvalue?id="+id, function( j ) {
              }).done(result => {
                  resolve(result);
              });
          });
    ...
    async function run(){
      ...
      function getComments(){
        return new Promise(function(resolve, reject){
          // query留言api，自動生成出留言
          $.getJSON('/videoDemo/get/comment' + window.location.search + '&start=1', function(json){
              resolve(json);
          })
        })
      }
      function renderComments(json){
        ...
        for (let j of json){
          ...
          result
            ...
            .find('a.red.labeled.icon.button').attr('value', '/videoDemo/post/like?id='+j['pk']).end()
    ...
    $('#questionnaire').click(function() {
        ...
        $.post( '/videoDemo/post/questionnaire' + window.location.search, Object.assign(res, postdata))
        .done(function(response) {
